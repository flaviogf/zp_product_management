...
namespace ZPProductManagement.Application.Products
{
    public class ActiveProduct
    {
        ...
        public ActiveProduct(IProductRepository productRepository, ICategoryRepository categoryRepository, IFileRepository fileRepository)
        {
        ...
        public async Task<Result> Execute(Guid productId)
        {
            ...
            var activeProductOrError = await ActiveProductOrError(getProductOrError.Value);

            if (activeProductOrError.Failure)
            {
                return Result.Fail(activeProductOrError.Message);
            }
